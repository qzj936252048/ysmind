...
<body style="background-color: white;">
		<div class="formbody" id="printPageId">
			<div class="formtitle" style="height: 50px;">
				...
				<span>请假申请（项目编号：${leaveApply.projectNumber}）</span>
				<span style="float: right;width: 200px;">发起人：${leaveApply.applyUser.name}&nbsp;</span>
				</div>
			</div>
		    ...
		    <table class="gridtable">
				<tr>
					...
					<td>${leaveApply.company.name}</td>
				</tr>
				<tr>
				    <td class="tdOne">部门：</td>
					<td>${leaveApply.office.name}</td>
				</tr>
				<tr>
				...
				<tr>
					...
					<td>${leaveApply.applyUser.name}</td>
				</tr>
				<tr>
				    <td class="tdOne">申请时间：</td>
					<td><fmt:formatDate value='${leaveApply.applyDate}' pattern="yyyy-MM-dd HH:mm:ss"/></td>
				</tr>
				<tr>
					<td class="tdOne">项目编号：</td>
					<td>${leaveApply.projectNumber}</td>
				</tr>
				<tr>
				    <td class="tdOne">工号：</td>
					<td>${leaveApply.applyUser.no}</td>
				</tr>
				<tr>
				    <td class="tdOne">职称：</td>
					<td>${leaveApply.applyUser.no}</td>
				</tr>
				<tr>
					<td class="tdOne">请假时长(H)：</td>
					<td >${leaveApply.leaveTotalTimes}</td>
				</tr>
			</table>
			</div> 
			<div style="border:dashed 1px gray;border-bottom: none;margin-top:-6px;border-top: none;">
			<div style="border-bottom: dashed 1px gray;padding-left: 10px;">请假申请明细</div>
				...
				<c:forEach items="${detailList}" var="detail" varStatus="status">
					<tr id="${detail.id}Tr" class="${detail.id}">
						...
						<td><fmt:formatDate value='${detail.startDate}' pattern="yyyy-MM-dd HH:mm"/></td>
						<td><fmt:formatDate value='${detail.endDate}' pattern="yyyy-MM-dd HH:mm"/></td>
						<td>${detail.leaveType }</td>
						<td>${detail.leaveReason }</td>
					</tr>
				</c:forEach>
				</tbody>
			</table>
